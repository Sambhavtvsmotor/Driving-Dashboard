...
def compute_vehicle_metrics(metrics_df: pd.DataFrame) -> dict:
    """Aggregate KPIs across all trips with detailed breakdown of harsh events."""
    ...
    return {
        ...
        # Harsh event breakdown
        "Hard Accel Events": metrics_df["Hard Accel"].sum(skipna=True),
        "Hard Brake Events": metrics_df["Hard Brake"].sum(skipna=True),
        "Jerk Events": metrics_df["jerk_events"].sum(skipna=True),
        "Torque Jerk Events": metrics_df["torque_jerk_events"].sum(skipna=True),
        "Total Harsh Events": metrics_df["total_harsh_events"].sum(skipna=True),
